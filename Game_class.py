...
from Directorys_Settings import H, f_vehicles, f_enemies, f_items, f_vfx, f_sfx # Importamos las variables y directorios que usaremos del módulo [ Directorys_Settings ]
...
class Enemy(Sprite): # Clase Enemigo
	...
	def __init__(self,num_carril): 
		...
		self.rect.center = ( num_carril, -(randint(200,550))) # Posicion Inicial = centramos el rectangulo en X con el carril y en Y aleatoreamente, para que se creen en distintas posiciones
	...
	def update(self):
		...
		if self.rect.top > H:         # Si el enemigo se sale de la pantalla se vuelve a posicionar y actualza sus valores para dar la impresion de que es un "nuevo enemigo"
			self.speed = randint(5,9) # Genera un valor disinto de velocidad, para que siempre que se salga de la pantalla obtenga una velocidad diferente
...
class Fuel(Sprite): # Clase Gasolina
	...
	power_up = Sound(join(f_sfx,'Fuel_PowerUp.ogg')) # se crea un sonido para reproducirlo cuando se recargue gasolina ( Propiedades de la clase )
	...
	def recharge(self):
		self.power_up.play() # Usamos el metodo .play() de la clase Sound() para reproducir el archivo de sonido
...
class Blood(Sprite): # Clase para Animacion Sangre
	...
	def update(self):
		...
		blood_speed = 4 		 # Podemos controlar la velocidad de reproduccion de la animacion
...
'''
# NO TOCAR :V EN PROCESO !!!

class Bonus(Sprite):  # Clase Objeto bonus
	def __init__(self,bonus):
		super().__init__()
	
kill_animation = pygame.sprite.Group()
kill = Blood(x,y)
kill_animation.add(kill)
kill_animation.update()
