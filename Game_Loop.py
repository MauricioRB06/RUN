...
from Game_class import Player, Enemy, Police_siren, Fuel, Score_goal, Bonus, Blood     # Importamos las clases que usaremos del módulo [ Game_class ]
...
def game_loop(car): # Blucle del juego
	...
	while game:
		...
		bonus_kill = pygame.sprite.spritecollide(player,bonus,False)
		if bonus_kill:
			kill = Blood(new_bonus.rect.x,new_bonus.rect.y)
		...
		if remaining_fuel > 1 and remaining_fuel <=70:                             # Si la gasolina es menor a 70
			...
			music.set_volume(0.5)                                                  # Dejamos el volumen normal
